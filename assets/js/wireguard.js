...
(function() {
	...
	window.wireguard = {
		...
		// Returns null if the input is not a valid 32-byte key.
		publicFromPrivate: function(b64priv) {
			try {
				var priv = base64ToKey(b64priv);
				if (priv.length !== 32) return null;
				return keyToBase64(generatePublicKey(priv));
			} catch (e) {
				return null;
			}
		}
